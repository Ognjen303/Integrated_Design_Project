...
def calculate_angle (x, y):
    ...
    return angle 
'''
Colour Masking+Moments based block detection
...
def detect_dropoff(f):
    ...
    if (dst_pt1):
        ...
        return None
'''
# forward_path and detect_dropoff not used in runtime in the final competition
def forward_path(s):
    """s:stream
    ...
    while frame_counter <= 10:
        ...
        destinations = [intersections[0], intermediate_checkpts[0], intersections[1], block] # Detect all the checkpoints run time
        # Stop if all the checkpoints are found
        if None not in destinations:
            return destinations
    return [(421, 275), (288, 396), (156, 522), block] # Harcoded checkpoints + detected block (failsafe)
...
def detect_dropoff(s):
    ...
    while frame_counter <= 10:
        ...
        return [(313, 222), (375, 166), (525, 331), (467, 389)] # Hardcoded drop off points (failsafe)