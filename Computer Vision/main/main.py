...
# Terminal command: 'ssh -L 8081:idpcam2.eng.cam.ac.uk:8080 aps85@gate.eng.cam.ac.uk'
stream = cv2.VideoCapture('http://localhost:8081/stream/video.mjpeg')

# Checkpoints of the arena - calibrate regularly
starting_location = (532, 175)
intersection1 = (421, 275)
ramp = (288, 396)
intersection2 = (156, 522)
original_destinations = [intersection1, ramp, intersection2, detect_block(stream)] # Archive a dummy path
bottom_ramp = intermediates(intersection2, ramp, 4)[1]

drop_off = [(380, 151), (315, 211), (475, 386), (536, 325)] # Corrected for parallex error 
# Blue top, Blue bottom, Red bottom, Red top - 'bottom' is white boxes closer to ramp, 'top' is white boxes closer to starting point
bottom_drop_off = intermediates(original_destinations[0], starting_location, 7)[0] # Checkpoint before dropping off in the 'bottom' boxes
...
while (red+blue) < blocks_to_attempt: # Blocks picked up < Blocks to attempt
    if len(drop_off) == 4:
        destinations = [intersection1, ramp, intersection2, detect_block(stream)]
    else:
        destinations = [ramp, intersection2, detect_block(stream)]
    ...
    if block_colour == 'blue':
        if blue == 1:
            destinations = [original_destinations[-2], bottom_ramp, original_destinations[-3], bottom_drop_off, intermediates(bottom_drop_off, drop_off[0])[0], drop_off[0]]
            del drop_off[0] 
        elif blue == 0:
            destinations = [original_destinations[-2], bottom_ramp, original_destinations[-3], top_drop_off, intermediates(top_drop_off, drop_off[0])[0], drop_off[0]]
            del drop_off[0] 
        # Mistaken colour detection fail safe
        elif blue == 2:
            if red == 1:
                destinations = [original_destinations[-2], bottom_ramp, original_destinations[-3], bottom_drop_off, intermediates(bottom_drop_off, drop_off[-1])[0], drop_off[-1]]
                del drop_off[-1] 
            elif red == 0:
                destinations = [original_destinations[-2], bottom_ramp, original_destinations[-3], top_drop_off, intermediates(top_drop_off, drop_off[-1])[0], drop_off[-1]]
                del drop_off[-1] 
        elif blue == 3:
            destinations = [original_destinations[-2], bottom_ramp, original_destinations[-3], bottom_drop_off, intermediates(bottom_drop_off, drop_off[-1])[0], drop_off[-1]]
            del drop_off[0] 
