...
"""Deprecated as the OpenCV localisation + pose estimation of the AruCo tags was too computationally intensive -
   latency builds up as the frames are added to a stack for processing"""
...
block_coordinates = deque([], maxlen = 5)

last_corners = None
last_ids = None
last_recoveredIds = None
...
while(True):
    ...
    if len(corners) != 0:
      ...
      last_ids = ids
      last_recoveredIds = recoveredIds

    if len(corners) == 0:
      corners = last_corners
      ids = last_ids
      recoveredIds = last_recoveredIds
    ...
    # Draw the vector
    if corners and len(block_coordinates) == 10: 
      ...
      block_x = sum([pair[0] for pair in block_coordinates]) / 5
      block_y = sum([pair[1] for pair in block_coordinates]) / 5
