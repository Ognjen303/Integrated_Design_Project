...
"""ssh -L 8081:idpcam2.eng.cam.ac.uk:8080 aps85@gate.eng.cam.ac.uk --> Authenticate"""

stream = cv2.VideoCapture('http://localhost:8081/stream/video.mjpeg')

# Stream dimensions: (760, 1016, 3)
# AruCo board?
# Documentation: https://docs.opencv.org/4.x/d9/d6a/group__aruco.html
...
# Load camera properties from calibration
...
while(True):
    ...
    if len(corners) > 0:
      ...
      #Rotation and Translation vectors will be used for angle calculations
    ...
    # Only the bottom left part of the image is checked for blocks
    red_mask[:380, :] = 0
    red_mask[380:, 508:] = 0
